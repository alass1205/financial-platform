...
async function main() {
  ...
  const fs = require('fs');
  fs.writeFileSync('complete-platform-with-vault.json', JSON.stringify(deploymentInfo, null, 2));
  console.log("💾 Informations complètes sauvegardées dans complete-platform-with-vault.json");
  ...
  console.log("Authorized Tokens:", [trgAddress, clvAddress, rooAddress]);
  console.log("Authorized NFTs:", [govAddress]);
}
