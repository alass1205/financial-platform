...
router.post('/order', async (req, res) => {
  try {
    ...
    try {
      const realBalance = await unifiedBalanceService.getWalletBalance(userAddress, upperAssetSymbol);
    ...
    if (type.toUpperCase() === 'SELL') {
      try {
        ...
        transferResult = await unifiedBalanceService.getWalletBalance(
          userAddress, 
          upperAssetSymbol, 
          parseFloat(quantity),
          'SELL'
